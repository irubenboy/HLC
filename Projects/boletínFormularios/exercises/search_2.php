    ...
    if(isset($_POST['search'])){ // Si se ha llegado a esta página desde el botón enviar
        ...
        if(!$conditions) {
            foreach($arrayPersonas as $p){
                ...
                && $usernameCondition
                && $ageCondition){
                    if(!isset($_POST['all']) && count($countries) > 0){
        ...
        <div class="container-form">
            <form action="search_2.php" method = "post">
                ...
                <div class="checkbox-container">
                        <input type="checkbox" name = "all" value = '%'>Todos los países
                        <?php
                        if(count($arrayCountries) > 0){
                            foreach($arrayCountries as $index => $country){
                                echo "<input type = 'checkbox' name = 'countries[]' value = '$index'>"  . $arrayCountries[$index];
                            }
                        }
                        ?>
                    </select>
