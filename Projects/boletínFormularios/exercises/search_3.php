...
<head>
    ...
    if(isset($_POST['search'])){ // Si se ha llegado a esta página desde el botón enviar
        if(isset($_POST['name']) && isset($_POST['age'])){ // si se ha declarado nombre
            ...
            $age = $_POST['age']; // edad
            $countries = isset($_POST['countries']) ? $_POST['countries'] : ''; // ciudad
            ...
            if(!$conditions) {
                foreach($arrayPersonas as $p){
                    ...
                    && $usernameCondition
                    && $ageCondition){
                        if(!isset($_POST['all']) && !empty($countries)){
...
<body>
    <main>
        ...
        <div class="container-form">
            <form action="search_3.php" method = "post">
                ...
                <div class="checkbox-container">
                        <input type = "radio" name = "all" value="%">Todos los países</option>
                        <?php
                        if(count($arrayCountries) > 0){
                            foreach($arrayCountries as $index => $country){
                                echo "<input type = 'radio' name = 'countries' value = '$index'>"  . $arrayCountries[$index];
                            }
                        }
                        ?>
                    </select>
